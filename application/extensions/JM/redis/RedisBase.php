...
include_once __DIR__ . '/ShmConfig.php';
...
abstract class RedisBase {
    ...
    protected $DisableFun = array(
        "INFO", "KEYS", "BLPOP", "MSETNX", "BRPOP", "RPOPLPUSH", "BRPOPLPUSH", "SMOVE", "SINTER", "SINTERSTORE", "SUNION", "SUNIONSTORE", "SDIFF", "SDIFFSTORE", "ZINTER", "ZUNION",
    ...
    /*
     * getSet特殊处理
     * $key1 原key,key必需为string类型
     * $key2 生成的新key
     * return 原来key的值
     */

    public function getSet($key1, $key2) {
        $redis = $this->ConnectTarget($key1);
        if ($redis->type($key1) == self::STRING) {
            $val = $redis->get($key1);
            $redisTarget = $this->ConnectTarget($key2);
            if (!$redisTarget->set($key2, $val)) {
                return false;
            } else {
                return $val;
            }
